...
#     parser.add_argument('--anpha', type= float, default= 0.6)
...
#     model = convnext_tiny(args= args).to(device)
#     image =Image.fromarray(cv2.resize(cv2.imread("data/000001.jpg"), (244,244)))
#     image = tfms(image).to(device)
#     dummy = torch.ones((3, 224, 224)).to(device)
#     input = torch.cat((image, dummy), dim= 1).to(device).unsqueeze(0)
#     model.eval() #non-gradient
#     output = model(input)
#     print(output)
#     output = F.softmax(output, dim=-1)
#     outputs_s = output[:, -1].mean().item()
#     print(outputs_s)
#     # print(torch.max(model(dummy, dummy), 1))